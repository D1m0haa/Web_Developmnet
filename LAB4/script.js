// перемещение, в большом количестве заимств.
...
var draw = function(ch, cls) {
  var f = fs.split('*')[now[0]].split(':')[now[1]].split('|').map(function(a) {
    return a.split('')
  });
...
var roll = function(ln) {
    if (false !== (document.querySelector('[data-y="' + ln + '"]').innerHTML = document.querySelector('[data-y="' + (ln - 1) + '"]').innerHTML) && ln > 1) roll(ln - 1);
  };
  ...
  if (e.keyCode == 40)
    if (false === draw([0, 1], undefined, deDraw())) {
      ...
      if (false === draw([0, 0], undefined, now = [Math.floor(Math.random() * fs.split('*').length), 0], pos = [4, 0])) {
        toV = -1;
        alert('Количество очков: ' + document.querySelector('#result').innerHTML);
      }
    }
});
var deDraw = function() {
    if (document.querySelectorAll('.now').length > 0) deDraw(document.querySelector('.now').classList.remove('now'));
  }
toF = function() {
  kdf({
    keyCode: 40
